...
# --------------------------------------------------------------------------- #
# SOR / NXT (넥스트레이드) 통합 라우팅 — 단일 출처
# --------------------------------------------------------------------------- #
# EXCG_ID_DVSN_CD body/param 값
EXCG_KRX = "KRX"
EXCG_NXT = "NXT"
EXCG_SOR = "SOR"
EXCG_ALL = "ALL"

# kis_market_routing config 값
MARKET_ROUTING_KRX = "krx"
MARKET_ROUTING_NXT = "nxt"
MARKET_ROUTING_SOR = "sor"
VALID_MARKET_ROUTINGS = (MARKET_ROUTING_KRX, MARKET_ROUTING_NXT, MARKET_ROUTING_SOR)

# 라우팅 → 주문/체결조회 EXCG_ID_DVSN_CD
EXCG_BY_ROUTING = {
    MARKET_ROUTING_KRX: EXCG_KRX,
    MARKET_ROUTING_NXT: EXCG_NXT,
    MARKET_ROUTING_SOR: EXCG_SOR,
}
# 라우팅 → 실시간 호가 WS TR_ID (KRX / NXT / 통합)
WS_ORDERBOOK_TR_ID_BY_ROUTING = {
    MARKET_ROUTING_KRX: "H0STASP0",
    MARKET_ROUTING_NXT: "H0NXASP0",
    MARKET_ROUTING_SOR: "H0UNASP0",
}
# 라우팅 → 실시간 체결가 WS TR_ID (KRX / NXT / 통합)
WS_TRADE_TR_ID_BY_ROUTING = {
    MARKET_ROUTING_KRX: "H0STCNT0",
...
#
# SOR → 'UN' (통합): EC2 라이브 프로브(JEP-180, 2026-06-19)에서 'UN'(통합)이 호가
# REST(inquire-asking-price-exp-ccn)에서 KRX 정규장 마감(15:30 KST)을 넘겨 NXT
# 애프터마켓(15:40-20:00 KST)까지 실시간 시세를 정상 스트리밍함을 확인. 이전 'UN
# 타임아웃' 관찰(JEP-162, 2026-06-18)은 재현되지 않음. 'J'(KRX)는 정규장 마감 후
# 동결되어 NXT 시간대에 fair가 stale → SOR은 'UN'으로 매핑. 주문 라우팅
# (EXCG_BY_ROUTING)은 SOR 그대로(EXCG_ID_DVSN_CD=SOR), 시세 READ 경로만 변경.
