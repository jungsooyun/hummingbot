...
class BithumbAPIOrderBookDataSource(OrderBookTrackerDataSource):
    def __init__(self, trading_pairs: List[str], connector: "BithumbExchange", api_factory: WebAssistantsFactory):
        super().__init__(trading_pairs)
        self._connector = connector
        self._api_factory = api_factory
        # Bithumb v2 WS orderbook channel emits full top-N snapshots.
    ...
    async def _subscribe_channels(self, websocket_assistant: WSAssistant):
        try:
            await self._send_orderbook_subscription(websocket_assistant=websocket_assistant, trading_pairs=self._trading_pairs)
            self.logger().info("Subscribed to Bithumb v2 public order book channels...")
        except asyncio.CancelledError:
            raise
        except Exception:
            self.logger().error(
                "Unexpected error occurred subscribing to Bithumb v2 order book streams...",
                exc_info=True,
            )
            raise

    async def _send_orderbook_subscription(self, websocket_assistant: WSAssistant, trading_pairs: List[str]):
        if len(trading_pairs) == 0:
            return
        codes = [await self._bithumb_code_for_pair(trading_pair) for trading_pair in trading_pairs]
        payload = [
            {"ticket": str(uuid.uuid4())},
            {
                "type": CONSTANTS.ORDERBOOK_EVENT_TYPE,
                "codes": codes,
                "level": 1,
            },
            {"format": "DEFAULT"},
    ...
    async def _bithumb_code_for_pair(self, trading_pair: str) -> str:
        symbol = await self._connector.exchange_symbol_associated_to_pair(trading_pair=trading_pair)
        if "." in symbol:
            return symbol
    ...
    async def _parse_trade_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        # Public Bithumb v2 trade stream is not used by this strategy.
        return

    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        # Bithumb orderbook events are full top-N snapshots; route as snapshots.
    ...
    async def subscribe_to_trading_pair(self, trading_pair: str) -> bool:
        ...
        try:
            ...
            await self._send_orderbook_subscription(self._ws_assistant, self._trading_pairs)
