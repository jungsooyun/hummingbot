"""
Integration tests for Upbit connector against the LIVE API.

Run with:
PYENV_VERSION=py312 python -m pytest test/hummingbot/connector/exchange/upbit/test_upbit_integration.py -v -m integration
"""
...
class TestUpbitIntegrationPublicAPI(unittest.TestCase):
    def _run(self, coro):
        return asyncio.get_event_loop().run_until_complete(coro)

    def test_market_all_response_structure(self):
        data = self._run(_fetch(CONSTANTS.GET_TRADING_RULES_PATH_URL, {"is_details": "true"}))
