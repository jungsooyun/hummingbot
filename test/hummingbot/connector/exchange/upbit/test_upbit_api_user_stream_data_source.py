...
class UpbitAPIUserStreamDataSourceTests(IsolatedAsyncioWrapperTestCase):
    level = 0
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.auth = UpbitAuth(access_key="test_access_key", secret_key="test_secret_key")

        self.connector = UpbitExchange(
        ...
        self.data_source = UpbitAPIUserStreamDataSource(
            auth=self.auth,
            trading_pairs=[self.trading_pair],
    ...
    async def test_subscribes_to_private_channels(self, ws_connect_mock):
        ...
        # Upbit subscription is a list: [ticket, myOrder, myAsset, format]
        self.assertIsInstance(payload, list)
        self.assertEqual(4, len(payload))
        self.assertIn("ticket", payload[0])
        self.assertEqual(CONSTANTS.PRIVATE_ORDER_CHANNEL_NAME, payload[1]["type"])
        self.assertIn(self.ex_trading_pair, payload[1]["codes"])
        self.assertEqual(CONSTANTS.PRIVATE_ASSET_CHANNEL_NAME, payload[2]["type"])
        self.assertEqual("DEFAULT", payload[3]["format"])
    ...
    async def test_does_not_queue_invalid_payload(self, mock_ws):
        ...
        # Message without matching type should be ignored
        event_no_type = {"status": "UP"}
        self.mocking_assistant.add_websocket_aiohttp_message(
            websocket_mock=mock_ws.return_value,
            message=json.dumps(event_no_type))

        # Message with unrelated type
        event_wrong_type = {"type": "orderbook", "code": "KRW-BTC"}
        self.mocking_assistant.add_websocket_aiohttp_message(
            websocket_mock=mock_ws.return_value,
            message=json.dumps(event_wrong_type))
    ...
    async def test_processes_multiple_events(self, mock_ws):
        ...
        self.assertEqual(order_event["type"], first_msg["type"])
        self.assertEqual(order_event["uuid"], first_msg["uuid"])
        self.assertIn(HB_DIAG_WS_RECV_TS_MS, first_msg)
        self.assertIn(HB_DIAG_WS_RECV_MONO_TS_MS, first_msg)
        self.assertEqual(asset_event["type"], second_msg["type"])
        self.assertEqual(asset_event["currency"], second_msg["currency"])
    ...
    async def test_raises_on_error_message(self, sleep_mock, mock_ws):
        ...
        error_event = {"error": "unauthorized", "message": "Invalid token"}
        self.mocking_assistant.add_websocket_aiohttp_message(
            websocket_mock=mock_ws.return_value,
